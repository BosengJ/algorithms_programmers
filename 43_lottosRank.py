...
def lottosRank(cnt_match):
    rank_list = [1,2,3,4,5,6]
    match_nums = [6,5,4,3,2,1]
    rank_dict = dict(zip(rank_list,match_nums))
    
    for rank,num in rank_dict.items():
        if num == cnt_match:
            return rank
        if cnt_match == 0:
            return 6
...
def solution(lottos, win_nums):
    ...
    for num in lottos:
        if num == 0:
            cnt_zero += 1
    top_rank_cnt_match = cnt_match + cnt_zero
    top_rank = lottosRank(top_rank_cnt_match)
    
    answer = [top_rank,low_rank]
    return answer